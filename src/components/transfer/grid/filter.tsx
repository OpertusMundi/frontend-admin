...
interface TransferFiltersProps extends WithStyles<typeof styles> {
  intl: IntlShape,
  query: TransferQuery,
  setFilter: (query: Partial<TransferQuery>) => void,
  resetFilter: () => void,
  find: (
...
class TransferFilters extends React.Component<TransferFiltersProps> {
            ...
            <Autocomplete
              ...
              renderInput={(params) => (
                <TextField
                  {...params}
                  variant="standard"
                  label={_t({ id: 'billing.transfer.filter.status' })}
                />
              )}
            />
          </Grid>
...
// Apply styles
const styledComponent = withStyles(styles)(TransferFilters);
