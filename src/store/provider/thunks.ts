...
export const find = (
  ...
  if (sorting) {
    ...
    sorting = getState().account.provider.sorting;
  }
  ...
  if (pageRequest) {
    ...
    pageRequest = getState().account.provider.pagination
  }
  ...
  const response = await api.findProviders(query, pageRequest, sorting);
